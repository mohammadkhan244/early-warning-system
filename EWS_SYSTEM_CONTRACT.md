...
Stage 7.5 — Reflection Fork (full specification)
The fork is a single question with two buttons. Not a rating. Not a form.
"Did this surface something you didn't already know — or confirm what you did?"
Button A: "Surfaced something new" → Push session booking. Standard CTA. → reflectionType = new
Button B: "Confirmed what I knew" → Do not push session booking. → Display two follow-up questions, static, in sequence:
"What's the part you keep avoiding acting on — and what's the smallest possible version of that action?"
"If someone you trusted read this story, what would they say you already know how to do?"
These questions are fixed. They are not generated by the API. They do not vary by stage or lens. They are the same for every user who selects "confirmed."
The fork is not a conversation. It does not adjust based on the user's answers. It asks. It stops. The answers are saved to feedbackLiked and feedbackWish and written to columns N and O in the Google Sheet.
Why these two questions: Both redirect from recognition to motion. The first surfaces the avoided move. The second borrows a trusted outside perspective without requiring one to be present. Neither requires more analysis of the story — they require the user to answer, which is the thing the tool's ceiling prevents it from doing on its own.
Non-negotiable: The fork must never become a chatbot. No follow-up to the follow-up. No API call on the confirmed path. Two questions, static, then the standard download CTA.
